...
export class Douyu extends Base {
  ...
  async getLiveLinks(): Promise<LiveLinks> {
    ...
    const mobileStream = await this.getMobileStream(params);
    if (mobileStream) {
      links.mobile = mobileStream;
    }

    const name = await this.getLiveNameForLinks(links);

    // 收集CDN链接
    for (const [cdn, config] of Object.entries(CDNS)) {
      for (const [prefix, format] of Object.entries(config)) {
        const linkPrefix = `http://${prefix}.${cdn}${name}.`;
        if (format.flv) {
          links.cdnLinks.flv.push(`${linkPrefix}flv`);
        }
        if (format.m3u8) {
          links.cdnLinks.m3u8.push(`${linkPrefix}m3u8`);
        }
      }
    }

    // 收集代理链接
    links.proxy = DOUYU_PROXY + this.roomID.toString();
  ...
  // 辅助方法：替代原getLiveName，仅收集PC端链接不打印
  private async getLiveNameForLinks(links: LiveLinks): Promise<string> {
    let info = await this.series(await this.params());
    let linkName = "";

    if (info && info.error !== -15) {
      if (info.data.rtmp_live === undefined) {
        throw new Error(`${this.finalRoomID} 房间未开播`);
      } else {
        linkName = info.data.rtmp_live.split("?")[0].split(".")[0].split("_")[0];
        links.pc = `${info.data.rtmp_url}/${info.data.rtmp_live}`; // 收集PC端链接
      }
    } else {
      this.isPost = !this.isPost;
      info = await this.series(await this.params());
      if (!info || info.data.rtmp_live === undefined) {
